<script lang="ts" module>
	...
	export const buttonVariants = tv({
		...
		variants: {
			...
			size: {
				// passanger brand contract (DESIGN.md / UX-DR1): sm 40 / md(default) 44 / lg 52,
				// radius 8px (rounded-md). Ends the ad-hoc h-[52px]/min-h-11 sizing.
				default:
					'h-11 gap-1.5 rounded-md px-4 has-data-[icon=inline-end]:pr-3 has-data-[icon=inline-start]:pl-3',
				xs: "h-6 gap-1 rounded-[min(var(--radius-md),10px)] px-2 text-xs in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pr-1.5 has-data-[icon=inline-start]:pl-1.5 [&_svg:not([class*='size-'])]:size-3",
				sm: 'h-10 gap-1.5 rounded-md px-3.5 has-data-[icon=inline-end]:pr-2.5 has-data-[icon=inline-start]:pl-2.5',
				lg: 'h-13 gap-2 rounded-md px-5 has-data-[icon=inline-end]:pr-4 has-data-[icon=inline-start]:pl-4',
...
{#if href}
	<!-- eslint-disable svelte/no-navigation-without-resolve -- generic UI primitive: href is a passthrough prop; the caller is responsible for passing a resolved path -->
	<a
		bind:this={ref}
		data-slot="button"
